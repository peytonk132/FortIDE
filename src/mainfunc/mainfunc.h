...
#include <boost/asio/read_until.hpp>  // Add this header
    ...
    void startAsyncProcess(const std::string& command, const std::string& type) 
    {
        ...
        buildFuture = std::async(std::launch::async, [this, command, type]() {
            try {
                ...
                std::function<void(const boost::system::error_code&, std::size_t)> readStdOut;
                std::function<void(const boost::system::error_code&, std::size_t)> readStdErr;

                // Define stdout handler
                readStdOut = [this, stdoutBuffer, &outPipe, &readStdOut](
                ...
                readStdErr = [this, stderrBuffer, &errPipe, &readStdErr](
                    const boost::system::error_code& ec, 
                    std::size_t bytes_transferred
                ) {
                    if (!ec && bytes_transferred > 0) {
