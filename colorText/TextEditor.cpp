...
const TextEditor::Palette& TextEditor::GetPurpleFortPalette()
{
	const static Palette p = { {
			0xff00ffff,	// None
			0xffffff00,	// Keyword	
...
static bool TokenizeFortranString(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	const char* p = in_begin;

	if (*p == '\'') // Fortran strings are enclosed in single quotes
	{
		p++;
		...
		while (p < in_end)
		{
			...
			// Handle escape character for single quote
			if (*p == '\\' && p + 1 < in_end && p[1] == '\'')
				p++;
...
static bool TokenizeFortranIdentifier(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	const char* p = in_begin;

	// Fortran identifiers start with a letter or underscore
	if ((*p >= 'a' && *p <= 'z') || (*p >= 'A' && *p <= 'Z') || *p == '_')
	{
		p++;

		// Continue as long as alphanumeric characters or underscore
		while ((p < in_end) && ((*p >= 'a' && *p <= 'z') || (*p >= 'A' && *p <= 'Z') || (*p >= '0' && *p <= '9') || *p == '_'))
			p++;
...
static bool TokenizeFortranPunctuation(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	(void)in_end;
		...
		langDef.mTokenize = [](const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end, TextEditor::PaletteIndex& paletteIndex) -> bool
			{
				...
				else if (TokenizeFortranString(in_begin, in_end, out_begin, out_end))
					paletteIndex = TextEditor::PaletteIndex::String;
				else if (TokenizeCStyleCharacterLiteral(in_begin, in_end, out_begin, out_end))
					paletteIndex = TextEditor::PaletteIndex::CharLiteral;
				else if (TokenizeFortranIdentifier(in_begin, in_end, out_begin, out_end))
					paletteIndex = TextEditor::PaletteIndex::Identifier;
				else if (TokenizeCStyleNumber(in_begin, in_end, out_begin, out_end))
					paletteIndex = TextEditor::PaletteIndex::Number;
				else if (TokenizeFortranPunctuation(in_begin, in_end, out_begin, out_end))
