...
int main() {
  ...
  try {
    ...
    if (auto existing = spdlog::get("build_logger")) {
      ...
      logger = std::make_shared<spdlog::logger>("build_logger", sinks.begin(),
                                                sinks.end());
  ...
  /*if
  (!io.Fonts->AddFontFromFileTTF("C:\\Users\\Peyton\\Downloads\\Open_Sans\\OpenSans-VariableFont_wdth,wght.ttf",
  8.0f)) { printf("Failed to load font1.ttf\n");
  }*/
  ...
  while (window.isOpen()) {
    ...
    ImVec2 fixedWidgetPosition =
        ImVec2(800, 30); // Fixed position for the widget
